...
MODE = "C"
dirs = 'interval_result/' + str(time.time())+MODEL_NAME
...
if __name__ == "__main__":
    ...
    model = linear_model.LinearRegression() #model = ensemble.RandomForestRegressor(n_estimators=20) # model = load_model("water/water/modelWights/weights_mobileNet_without_pre1570856691.343963_color.h5")
    empty_model = linear_model.LinearRegression()

    #result, predicts, y_interval = new_model(model，empty_model, mode = MODE,inversed=False, X_train, y_train, X_test, y_test)
    ...
    #u_model,l_model = load_smoother(LOAD_PATH)
