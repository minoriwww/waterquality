...
def test_interval_accuracy(predictions, u_model, l_model, y_test):

    # load range model and predict upper and lower offsets
    upper_offset = u_model.predict(np.array(predictions, np.float).reshape((-1, 1)))[0]
    lower_offset = l_model.predict(np.array(predictions, np.float).reshape((-1, 1)))[0]

    lower_list = predictions - lower_offset
    upper_list =  predictions + upper_offset
    correction = 0.0
    #tag = 0
    for idx in range(len(y_test)):
        if lower_list[idx] < 0:
            lower_list[idx] = 0
        if upper_list[idx] > 500:
            upper_list[idx] = 500
        if  y_test[idx] >= lower_list[idx] and y_test[idx] <= upper_list[idx]:
            correction += 1.0
            tag = 1
        else:
            tag = 0
        print('lower:{:f} upper:{:f} gt:{:f} tag:{:d}'.format(lower_list[idx], upper_list[idx], y_test[idx], tag))
    accuracy = correction / len(y_test)
    print('after post-processing: correct:{:f} total:{:f} accuracy:{:f} '.format(correction, len(y_test), accuracy))
...
if __name__ == "__main__":
    ...
    #predicts = transform_y(predicts, scaler_val)
    #result = transform_y(result, scaler_val)
    np.savetxt('train_gt.txt', y_interval.reshape((-1)), fmt='%.3f', delimiter='\n')
    # np.savetxt('train_gt.txt', y_train.reshape((-1)), fmt='%.3f', delimiter='\n')
    # np.savetxt('train_results.txt', predicts.reshape((-1)), fmt= '%.3f', delimiter='\n')
    ...
    #u_model,l_model = load_smoother(r'interval_result/1573514984.6583798MobileNet')

    #draw_test(result, y_test,final_upper,final_lower,True,u_model,l_model)
    #test_interval_accuracy(result,u_model,l_model, y_test)
