...
def train_and_predict( batch_size,
    ...
    model_checkpoint = ModelCheckpoint('./modelWights/'+weight_file, monitor='val_loss', save_best_only=True)
    ...
    model = load_model('./modelWights/'+model_file)
...
def show_roc_pr_curve(model_name = "",
    ...
    lw = 2
    plt.plot(fpr, tpr,lw=lw, label='ROC (auc = %0.2f)' % roc_auc)
...
if __name__ == '__main__':
    ...
    train_and_predict( BATCHSIZE,
        weight_file = 'weightsVGG.h5'
        # model_file = 'model_VGG16.h5'
        )
    # compare()
